...
func (k msgServer) HireAgent(goCtx context.Context, msg *types.MsgHireAgent) (*types.MsgHireAgentResponse, error) {
	...
	if msg.Budget < 100 {
		return nil, fmt.Errorf("minimum hire budget is 100 uoan")
	}
	...
	ownerCoins := sdk.NewCoins(sdk.NewInt64Coin("uoan", int64(ownerAmount)))
	...
	if feeAmount > 0 {
		feeCoins := sdk.NewCoins(sdk.NewInt64Coin("uoan", int64(feeAmount)))
